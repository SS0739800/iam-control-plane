...
Small on purpose. Every SCIM handler goes through here so the media type and the
list envelope can't drift apart between endpoints, which is the kind of thing
that only shows up against the one provider that checks.
"""
...
def scim_json(document: dict[str, Any], status_code: int = status.HTTP_200_OK) -> JSONResponse:
    ...
    response that claims to be plain JSON, which looks from our side like a sync
    that ran fine and did nothing.
    """
...
def resource_json(resource: BaseModel, status_code: int = status.HTTP_200_OK) -> JSONResponse:
    ...
    ``exclude_none`` because SCIM would rather a field were absent than null, and
    ``by_alias`` because the wire wants camelCase while our code uses Python names.
    """
    return scim_json(
...
def list_json(resources: Sequence[BaseModel], *, total: int, start_index: int) -> JSONResponse:
    ...
    ``itemsPerPage`` is how many are actually in this response, not how many were
    asked for. A provider that reads the requested size instead keeps paging past
...
def paging(start_index: int | None, count: int | None) -> tuple[int, int]:
    ...
    startIndex is 1-based and the spec says a value below 1 is treated as 1
    rather than rejected. Getting that wrong by one either skips the first person
    in the directory or loops forever on the same page.
    """
...
async def scim_error_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    Registered in main.py so no handler builds the envelope itself. The signature
    takes a bare Exception because that is what Starlette's handler protocol
    says; the isinstance check is a formality that also keeps mypy happy.
    """
