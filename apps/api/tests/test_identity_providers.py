...
def register(
    ...
    **extra: object,
    ...
    return client.post(
        ...
        json=body,
        headers=console.as_user(as_user or console.admin),
    )
...
def test_registering_hands_back_the_login_link(
    db_client: TestClient, console: ConsoleUsers
) -> None:
...
def test_nothing_is_registered_when_the_metadata_is_bad(
    ...
    listed = db_client.get("/api/identity-providers", headers=console.as_admin).json()
...
def test_an_auditor_can_see_which_providers_are_registered(
    ...
    response = db_client.get("/api/identity-providers", headers=console.as_user(console.auditor))

    assert response.status_code == 200
    assert any(provider["slug"] == console.slug for provider in response.json())


def test_an_auditor_cannot_register_one(db_client: TestClient, console: ConsoleUsers) -> None:
...
def test_an_employee_cannot_even_look(db_client: TestClient, console: ConsoleUsers) -> None:
    response = db_client.get("/api/identity-providers", headers=console.as_user(console.employee))

    assert response.status_code == 403


def test_the_list_leaves_out_the_certificate(db_client: TestClient, console: ConsoleUsers) -> None:
...
def test_asking_for_one_provider_includes_the_certificate(
    ...
    response = db_client.get(f"/api/identity-providers/{console.slug}", headers=console.as_admin)
...
def test_asking_for_a_provider_that_is_not_there_is_a_404(
    ...
    response = db_client.get("/api/identity-providers/never-registered", headers=console.as_admin)
